...
node(Id, Predecessor, Successor, Store) ->
    receive
        ...
        {lookup, Key, Qref, Client, Hops} ->
            lookup(Key, Qref, Client, Id, Predecessor, Successor, Store, Hops),
            node(Id, Predecessor, Successor, Store);
        {handover, Elements} ->
...
lookup(Key, Qref, Client, Id, {Pkey, _}, Successor, Store, Hops) ->
    % if client key is between pred key and our key (id) then we do the lookup else pass to successor
    case key:between(Key, Pkey, Id) of
        true ->
            Result = storage:lookup(Key, Store),
            Client ! {Qref, Result},
            io:format("~n Hops = ~p ~n",[Hops]),
            Hops;
            
        false ->
            {_, Spid} = Successor,
            Spid ! {lookup, Key, Qref, Client, Hops+1}
