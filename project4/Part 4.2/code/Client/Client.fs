...
//websockets declarations for methods
...
let onRx nodeName = fun (arg:MessageEventArgs) ->
    ...
    // fetch the data by applying Deserialize
    ...
    | "Register" -> if resp.Status = "Success" then
                        printfn "Register Suceeded for %A \n" nodeName
                        actorRef <! "Logout"
                    else
                        printfn "Register failed with reason: %s \n" resp.ReasonForError
    | "Logout" -> if resp.Status = "Success" then
                       actorRef <! "Login"
                  else 
                      printfn "Logout failed with reason: %s \n" resp.ReasonForError
    | "Login" -> if resp.Status = "Success" then
                    actorRef <! "Tweet"
                 else 
                      printfn "Login failed with reason: %s \n" resp.ReasonForError
    | "Tweet" -> if resp.Status = "Success" then
                    total <- total + 10
                    if total = 10 then
                        actorRef <! "QueryMen"
                 else 
                      printfn "Tweet failed with reason: %s \n" resp.ReasonForError
    | "QueryMentions" -> if resp.Status = "Success" then
                            let res = JsonSerializer.Deserialize<List<string>>(resp.Result)
                            printfn "Got the queryMentions result: %A \n" res
                            actorRef <! "QueryHash"
                         else 
                            printfn "QueryMentions failed with reason: %s \n" resp.ReasonForError
    | "QueryHashTags" -> if resp.Status = "Success" then
                            let res = JsonSerializer.Deserialize<List<string>>(resp.Result)
                            printfn "Got the queryHashtags result: %A \n" res
                            actorRef <! "Retweet"
                         else 
                            printfn "QueryHashTags failed with reason: %s \n" resp.ReasonForError
    | "Retweet" -> if resp.Status = "Success" then
                        printfn "Retweet success \n"
                        actorRef <! "QueryTweets"
                   else
                        printfn "Retweet failed with reason: %s \n" resp.ReasonForError
    | "QueryTweets" -> if resp.Status = "Success" then
                            let res = JsonSerializer.Deserialize<List<string>>(resp.Result)
                            printfn "Got the query tweets result: %A \n" res
                       else 
                            printfn "QueryTweets Failed failed with reason: %s \n" resp.ReasonForError
    | "TweetLive" -> if resp.Status = "Success" then
                        let res = JsonSerializer.Deserialize<List<string>>(resp.Result)
                        printfn "Client %s tweeted %s \n" nodeName res.[0]
    | _ -> printfn "Wrong tag \n"

let error = fun (arg:ErrorEventArgs) ->
    printfn "Request failed with %A \n" arg.Message
