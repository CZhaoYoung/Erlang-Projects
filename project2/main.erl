...
run(Actor, Topology, Algorithm) ->
  % {ok, Actor} = {ok, 30},
  % {ok, Topology} = io:read("Topology: \n"),
  % {ok, Algorithm} = io:read("Input Algo: "),
  ...
  if
    ...
    Algorithm == "Push" ->
      ...
      % ActorList = [spawn(push_sum, spawn_actors, [1, 1, 0]) || _ <- lists:seq(1, Actor)];
    true -> ActorList =[]
  end,
...
check_for_convergence_condition(0, Start_Time) ->
  ...
  io:format("End Time ~w \n", [End_Time]),
  io:format("Convergence time is ~w milliseconds  ~n", [End_Time - Start_Time]),

  % unregister
  unregister(convergence),
  unregister(pass_message_to_neighbours);
